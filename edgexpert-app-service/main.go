...
import (
	...
	"github.com/andresrinivasan/simple-app-service/config"
	"github.com/andresrinivasan/simple-app-service/functions"
)
...
func main() {
	// TODO: See https://docs.iotechsys.com/Content/UserGuide/Layers/CH-ApplicationService.html
...
func (app *myApp) CreateAndRunAppService(serviceKey string, newServiceFactory func(string) (interfaces.ApplicationService, bool)) int {
	...
	// For more details see https://docs.edgexfoundry.org/2.3/microservices/application/GeneralAppServiceConfig/#writable-custom-configuration
	...
	//       See https://docs.iotechsys.com/edge-xpert22/app-services/SDK/built-in-pipeline-functions.html for list of built-in functions
	//
	...
	// TODO: Remove adding functions pipeline by topic if default pipeline above is all your Use Case needs.
	//       Or remove default above if your use case needs multiple pipelines by topic.
	// Example of adding functions pipelines by topic.
	// These pipelines will only execute if the specified topic match the incoming topic and the specified trigger type
	// match the type of trigger of incoming data.
	...
	// See https://docs.iotechsys.com/edge-xpert22/app-services/multiplepipelines/multiple-pipelines-examples.html for more details.
	...
	// TODO: In the case of reading functions pipeline settings from configuration, uncomment the code below and remove
	//       the above SetDefaultFunctionsPipeline and AddFunctionsPipelineForTopic invocations.
	//configuredPipelines, err := app.service.LoadConfigurableFunctionPipelines()
	//if err != nil {
	//	app.lc.Errorf("LoadConfigurableFunctionPipelines returned error: %s", err.Error())
	//	os.Exit(-1)
	//}
	//for _, pipeline := range configuredPipelines {
	//	switch pipeline.Id {
	//	case interfaces.DefaultPipelineId:
	//		if err = app.service.SetDefaultFunctionsPipeline(pipeline.Transforms...); err != nil {
	//			app.lc.Errorf("Unable to Set Default Functions Pipeline: %s", pipeline.Id, err.Error())
	//			os.Exit(-1)
	//		}
	//	default:
	//		if err = app.service.AddFunctionsPipelineForTopics(pipeline.Id, pipeline.Topics, pipeline.TriggerType, pipeline.UseTargetTypeOfByteArray, pipeline.Transforms...); err != nil {
	//			app.lc.Errorf("Unable to Add Functions Pipeline for pipeline id '%s': %s", pipeline.Id, err.Error())
	//			os.Exit(-1)
	//		}
	//	}
	//}
