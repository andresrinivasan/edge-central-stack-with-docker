...
var dataToBatch = []string{"Test1", "Test2", "Test3"}

func TestBatchNoData(t *testing.T) {

	bs, _ := NewBatchByCount(1, false)
...
func TestBatchInCountMode_UseRawDataType(t *testing.T) {

	bs, _ := NewBatchByCount(3, true)

	continuePipeline1, _ := bs.Batch(ctx, dataToBatch[0])
	assert.False(t, continuePipeline1)
	assert.Len(t, bs.batchData.all(), 1, "Should have 1 record")

	continuePipeline2, _ := bs.Batch(ctx, dataToBatch[1])
	assert.False(t, continuePipeline2)
	assert.Len(t, bs.batchData.all(), 2, "Should have 2 records")

	continuePipeline3, result3 := bs.Batch(ctx, dataToBatch[2])
	assert.True(t, continuePipeline3)
	assert.Len(t, result3, 3, "Should have 3 records")
	for i, v := range result3.([]interface{}) {
		assert.Equal(t, dataToBatch[i], v.(string))
	}
	assert.Len(t, bs.batchData.all(), 0, "Records should have been cleared")
}
func TestBatchInTimeAndCountMode_TimeElapsed(t *testing.T) {

	bs, _ := NewBatchByTimeAndCount("2s", 10, false)
	...
	go func() {
		...
		if continuePipeline1 {
			assert.Equal(t, 3, len(result.([]interface{})))
...
func TestBatchInTimeAndCountMode_CountMet(t *testing.T) {
	...
	bs, _ := NewBatchByTimeAndCount("90s", 3, false)
	var wgAll sync.WaitGroup
	var wgFirst sync.WaitGroup
...
func TestBatchIsEventData(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			bbc, err := NewBatchByCount(3, false)
			require.NoError(t, err)
...
func TestBatch_MergeOnSend(t *testing.T) {
	...
	bbc, err := NewBatchByCount(len(dataToBatch), false)
	require.NoError(t, err)
