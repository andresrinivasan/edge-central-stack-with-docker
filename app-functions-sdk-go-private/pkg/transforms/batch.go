...
func (d *atomicBatchData) append(toBeAdded interface{}) []interface{} {
	d.mutex.Lock()
...
func (d *atomicBatchData) all() []interface{} {
	d.mutex.Lock()
	defer d.mutex.Unlock()
	result := d.data
	return result
...
func NewBatchByTime(timeInterval string, useRawDataType bool) (*BatchConfig, error) {
	config := BatchConfig{
...
func NewBatchByCount(batchThreshold int, useRawDataType bool) (*BatchConfig, error) {
	config := BatchConfig{
...
func NewBatchByTimeAndCount(timeInterval string, batchThreshold int, useRawDataType bool) (*BatchConfig, error) {
	config := BatchConfig{
		timeInterval:   timeInterval,
		batchThreshold: batchThreshold,
...
func (batch *BatchConfig) Batch(ctx interfaces.AppFunctionContext, data interface{}) (bool, interface{}) {
	...
	if batch.IsEventData || !batch.useRawDataType {
		// convert data to []byte when IsEventData is true or useRawDataType is false, note that IsEventData will take
		// precedence over useRawDataType
		data, err = util.CoerceType(data)
		if err != nil {
			return false, err
		}
	}
	...
	if batch.batchData.length() > 0 {
		...
		if batch.IsEventData {
			...
			var mergedData []byte
			for _, data := range copyOfData {
				// only []byte can be merged
				if dataInBytes, ok := data.([]byte); ok {
					mergedData = append(mergedData, dataInBytes...)
				}
			}
