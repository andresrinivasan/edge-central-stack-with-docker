...
type PipelineFunction struct {
	...
	// Placeholder acts as a placeholder to work around https://github.com/edgexfoundry/app-functions-sdk-go/issues/936, so that a function without any parameters can still be written into consul.
	Placeholder string
}
...
// Types transforms a comma separated list of trigger types to map[string]string with uppercase keys and values
func (t TriggerInfo) Types() map[string]string {
	types := util.DeleteEmptyAndTrim(strings.FieldsFunc(t.Type, util.SplitComma))
