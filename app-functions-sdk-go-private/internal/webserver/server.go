...
func (webserver *WebServer) listenAndServe(serviceTimeout time.Duration, errChannel chan error) {
	...
	svr := &http.Server{
		Addr: addr,
		// Move http.TimeoutHandler to http.Server struct to fix go-lint error(G114: Use of net/http serve function that has no support for setting timeouts (gosec))
	...
	if config.HttpServer.Protocol == "https" {
		...
		if err != nil {
			lc.Errorf("unable to find HTTPS Secret %s in Secret Store: %v", config.HttpServer.SecretName, err)
			errChannel <- err
			return
		}
