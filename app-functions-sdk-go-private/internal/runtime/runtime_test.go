...
func TestProcessMessageBusRequest(t *testing.T) {
	...
	config.Trigger.Type = internal.TriggerTypeMessageBus
	runtime := NewGolangRuntime("", sdkCommon.NewAtomicTargetType(nil), dic)
	runtime.TargetTypeMap[interfaces.DefaultPipelineId] = sdkCommon.NewAtomicTargetType(nil)
	runtime.SetDefaultFunctionsPipeline([]interfaces.AppFunction{dummyTransform})

	_, decodeErr, _ := runtime.DecodeMessage(context, envelope, interfaces.DefaultPipelineId)
...
func TestProcessMessageNoTransforms(t *testing.T) {
	...
	messageData, decodeError, _ := runtime.DecodeMessage(context, envelope, interfaces.DefaultPipelineId)
...
func TestProcessMessageTransformError(t *testing.T) {
	...
	runtime := NewGolangRuntime("", sdkCommon.NewAtomicTargetType(&config.RegistryInfo{}), dic)
	runtime.TargetTypeMap[interfaces.DefaultPipelineId] = sdkCommon.NewAtomicTargetType(&config.RegistryInfo{})
...
func TestDecode_Process_MessageTargetType(t *testing.T) {
	...
	for _, currentTest := range targetTypeTests {
		t.Run(currentTest.Name, func(t *testing.T) {
			...
			runtime := NewGolangRuntime("", sdkCommon.NewAtomicTargetType(currentTest.TargetType), dic)
			runtime.TargetTypeMap[interfaces.DefaultPipelineId] = sdkCommon.NewAtomicTargetType(currentTest.TargetType)
			runtime.SetDefaultFunctionsPipeline([]interfaces.AppFunction{transforms.NewResponseData().SetResponseData})

			targetData, err, _ := runtime.DecodeMessage(context, envelope, interfaces.DefaultPipelineId)
...
func TestGetPipelineById(t *testing.T) {
	...
	expectedTriggerType := internal.TriggerTypeMessageBus
	expectedUseTargetTypeOfByteArray := false
	badId := "bogus"

	config := container.ConfigurationFrom(dic.Get)
	config.Trigger.Type = expectedTriggerType
	target.SetDefaultFunctionsPipeline(expectedTransforms)

	err := target.AddFunctionsPipeline(expectedId, expectedTopics, expectedTriggerType, expectedUseTargetTypeOfByteArray, expectedTransforms)
	require.NoError(t, err)
	...
	assert.Equal(t, expectedTriggerType, actual.TriggerType)
	assert.Equal(t, expectedUseTargetTypeOfByteArray, actual.UseTargetTypeOfByteArray)
...
func TestGetMatchingPipelines(t *testing.T) {
	...
	err := target.AddFunctionsPipeline("one", []string{"edgex/events/#/D1/#"}, internal.TriggerTypeMessageBus, false, expectedTransforms)
	require.NoError(t, err)
	err = target.AddFunctionsPipeline("two", []string{"edgex/events/P1/#"}, internal.TriggerTypeMessageBus, false, expectedTransforms)
	require.NoError(t, err)
	err = target.AddFunctionsPipeline("three", []string{"edgex/events/P1/D1/S1"}, internal.TriggerTypeMessageBus, false, expectedTransforms)
	require.NoError(t, err)
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			actual := target.GetMatchingPipelines(test.incomingTopic, internal.TriggerTypeMessageBus)
...
func TestGolangRuntime_SetFunctionsPipelineTransforms(t *testing.T) {
	...
	err := target.AddFunctionsPipeline(id, topics, internal.TriggerTypeMessageBus, false, initialTransforms)
	require.NoError(t, err)
...
func TestGolangRuntime_ClearAllFunctionsPipelineTransforms(t *testing.T) {
	...
	err := target.AddFunctionsPipeline(id1, topics, internal.TriggerTypeMessageBus, false, transforms)
	require.NoError(t, err)
	err = target.AddFunctionsPipeline(id2, topics, internal.TriggerTypeMessageBus, false, transforms)
	require.NoError(t, err)
