...
func TestProcessRetryItems(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.Name, func(t *testing.T) {
			...
			if test.UsePerTopic {
				err := runtime.AddFunctionsPipeline("per-topic", []string{"#"}, internal.TriggerTypeMessageBus, false, []interfaces.AppFunction{transformPassthru, transformPassthru, test.TargetTransform})
...
func TestDoStoreAndForwardRetry(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.Name, func(t *testing.T) {
			...
			if test.UsePerTopic {
				err := runtime.AddFunctionsPipeline("per-topic", []string{"#"}, internal.TriggerTypeMessageBus, false, []interfaces.AppFunction{transformPassthru, test.TargetTransform})
				require.NoError(t, err)
				pipeline = runtime.GetPipelineById("per-topic")
				require.NotNil(t, pipeline)
			} else {
