...
func (sf *storeForwardInfo) retryExportFunction(item interfaces.StoredObject, pipeline *interfaces.FunctionPipeline) bool {
	...
	// When MQTT-related export functions are specified in the pipeline, SharedMQTTClient will be used during retry,
	// so set SharedMQTTClient to appContext.  Note that we don't have to check if &sf.runtime.SharedMQTTClient is nil
	// or not as taking the address of a variable always yields a non-nil pointer.
	appContext.SetSharedMQTTClient(&sf.runtime.SharedMQTTClient)

	// When MQTT-related export functions are specified in the pipeline, SharedMQTTClientMutex will be used during retry,
	// so set SharedMQTTClientMutex to appContext.  Note that we don't have to check if &sf.runtime.SharedMQTTClient is
	// nil or not as taking the address of a variable always yields a non-nil pointer.
	appContext.SetSharedMQTTClientMutex(&sf.runtime.SharedMQTTClientMutex)
