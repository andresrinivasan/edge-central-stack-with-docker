...
func (c *Controller) sendResponse(
	...
	var data []byte
	var err error
	if request.Header.Get(common.ContentType) == common.ContentTypeTOML {
		data, err = toml.Marshal(response)
		writer.Header().Set(common.ContentType, common.ContentTypeTOML)
	} else {
		data, err = json.Marshal(response)
		writer.Header().Set(common.ContentType, common.ContentTypeJSON)
	}
	if err != nil {
		c.lc.Errorf("Unable to marshal %s response: %v, %s=%s", api, err, common.CorrelationHeader, correlationID)
	...
	if err != nil {
		c.lc.Errorf("Unable to write %s response: %v, %s=%s", api, err, common.CorrelationHeader, correlationID)
		http.Error(writer, err.Error(), http.StatusInternalServerError)
		return
	}
}
