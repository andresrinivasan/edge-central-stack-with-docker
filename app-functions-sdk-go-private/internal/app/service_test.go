	...
	sdk := Service{
		config: &common.ConfigurationStruct{
			Trigger: common.TriggerInfo{
				Type: internal.TriggerTypeMessageBus,
...
func TestSetupHTTPTrigger(t *testing.T) {
	...
	testRuntime := runtime.NewGolangRuntime("", common.NewAtomicTargetType(nil), dic)
...
func TestSetDefaultFunctionsPipelineOneTransform(t *testing.T) {
	...
	service := Service{
		lc:         lc,
		dic:        dic,
		targetType: common.NewAtomicTargetType(nil),
...
func TestService_AddFunctionsPipelineForTopics(t *testing.T) {
	config := &common.ConfigurationStruct{
		Trigger: common.TriggerInfo{
			Type: strings.Join([]string{internal.TriggerTypeMessageBus, "CUSTOM TRIGGER"}, ","),
		},
	}
	service := Service{
		lc:         lc,
	...
	tests := []struct {
		...
		{"Happy Path", "123", internal.TriggerTypeMessageBus, defaultTopics, transforms, false},
		{"Duplicate Id", interfaces.DefaultPipelineId, internal.TriggerTypeMessageBus, defaultTopics, transforms, true},
		{"Happy Path Custom", "124", "CUSTOM TRIGGER", defaultTopics, transforms, false},
		{"Empty Topic", "125", internal.TriggerTypeMessageBus, []string{" "}, transforms, true},
		{"Empty Topics", "126", internal.TriggerTypeMessageBus, nil, transforms, true},
		{"No Transforms", "127", internal.TriggerTypeMessageBus, defaultTopics, nil, true},
		{"Wrong Trigger Type", "128", internal.TriggerTypeHTTP, defaultTopics, transforms, true},
	}

	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			err := service.AddFunctionsPipelineForTopics(test.id, test.topics, test.trigger, false, test.transforms...)
			if test.expectError {
				require.Error(t, err)
				return
			}
...
func TestUseTargetTypeOfByteArrayTrue(t *testing.T) {
	...
	defaultTargetType := common.NewAtomicTargetType(nil)
	sdk := Service{
		lc:         lc,
		targetType: defaultTargetType,
		config:     config,
		runtime:    runtime.NewGolangRuntime("", defaultTargetType, dic),
	}
	dic.Update(di.ServiceConstructorMap{
		container.ConfigurationName: func(get di.Get) interface{} {
			return config
	...
	defaultTransforms, err := sdk.LoadConfigurablePipeline()
	require.NoError(t, err)
	err = sdk.SetDefaultFunctionsPipeline(defaultTransforms...)
	require.NoError(t, err)
	require.NotNil(t, sdk.targetType)
	assert.Equal(t, reflect.Ptr, reflect.TypeOf(sdk.targetType).Kind())
	assert.Equal(t, reflect.TypeOf([]byte{}).Kind(), reflect.TypeOf(sdk.runtime.TargetTypeMap[interfaces.DefaultPipelineId].Type()).Elem().Kind())
}
...
func TestUseTargetTypeOfByteArrayFalse(t *testing.T) {
	...
	err = sdk.SetDefaultFunctionsPipeline(defaultTransforms...)
	require.NoError(t, err)
	assert.Nil(t, sdk.runtime.TargetTypeMap[interfaces.DefaultPipelineId].Type())
}
...
func TestFindMatchingFunction(t *testing.T) {
	...
	configurable := reflect.ValueOf(NewConfigurable(svc.lc, svc.config.Trigger.ExternalMqtt))
...
func TestAddFunctionsPipelineForTopicsTriggerTypeNotFound(t *testing.T) {
	service := Service{
		lc:         lc,
		targetType: common.NewAtomicTargetType(nil),
		runtime:    runtime.NewGolangRuntime("", nil, dic),
	...
	err := service.AddFunctionsPipelineForTopics("123", []string{"#"}, internal.TriggerTypeMQTT, false, transforms...)
	require.Error(t, err)
	expectedErrMsg := fmt.Sprintf("the given trigger type %s could not be found in the [Trigger] configuration",
		internal.TriggerTypeMQTT)
	assert.Containsf(t, err.Error(), expectedErrMsg, "unexpected error message")
}

func TestSetTargetTypePerPipeline(t *testing.T) {
	...
	tests := []struct {
		...
		defaultTargetType        interface{}
		expectedType             interface{}
	}{
		{"default target type, useTargetTypeOfByteArry = false", false, nil, nil},
		{"default target type, useTargetTypeOfByteArry = true", true, nil, &[]byte{}},
		{"custom type, useTargetTypeOfByteArry = false", false, &customType{}, &customType{}},
		{"custom type, useTargetTypeOfByteArry = true", true, &customType{}, &[]byte{}},
	}

	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			svc := Service{
				targetType: common.NewAtomicTargetType(test.defaultTargetType),
				runtime:    runtime.NewGolangRuntime("", nil, dic),
			}
			svc.setTargetTypePerPipeline(pipelineID, test.useTargetTypeOfByteArray)
			tp, ok := svc.runtime.TargetTypeMap[pipelineID]
			require.True(t, ok)
			assert.Equal(t, test.expectedType, tp.Type())
		})
	}
}
