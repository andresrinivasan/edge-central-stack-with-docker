...
func NewService(serviceKey string, targetType interface{}, profileSuffixPlaceholder string) *Service {
	return &Service{
		...
		targetType:               common.NewAtomicTargetType(targetType),
...
type Service struct {
	dic                        *di.Container
	serviceKey                 string
	targetType                 *common.AtomicTargetType
...
func (svc *Service) AddBackgroundPublisherWithTopic(capacity int, topic string) (interfaces.BackgroundPublisher, error) {
	...
	if svc.config.Trigger.Type == internal.TriggerTypeHTTP || svc.config.Trigger.Type == internal.TriggerTypeMQTT {
...
func (svc *Service) MakeItRun() error {
	...
	// determine input type and create trigger
	triggers, err := svc.setupTriggers()
	if err != nil {
		return fmt.Errorf("failed to setup triggers. Error: %s", err)
	}

	for triggerType, trigger := range triggers {
		var deferred bootstrap.Deferred

		// Initialize the trigger (i.e. start a web server, or connect to message bus)
		if triggerType == internal.TriggerTypeMessageBus {
			deferred, err = trigger.Initialize(svc.ctx.appWg, svc.ctx.appCtx, svc.backgroundPublishChannel)
		} else {
			deferred, err = trigger.Initialize(svc.ctx.appWg, svc.ctx.appCtx, nil)
		}
		if err != nil {
...
func (svc *Service) LoadConfigurableFunctionPipelines() (map[string]interfaces.FunctionPipeline, error) {
	...
	if svc.config.Writable.Pipeline.UseTargetTypeOfMetric {
		svc.targetType = common.NewAtomicTargetType(&dtos.Metric{})
	}
	...
	if len(pipelineConfig.PerTopicPipelines) > 0 {
		for _, perTopicPipeline := range pipelineConfig.PerTopicPipelines {
			...
			pipeline := interfaces.FunctionPipeline{
				...
				TriggerType:              perTopicPipeline.TriggerType,
				UseTargetTypeOfByteArray: perTopicPipeline.UseTargetTypeOfByteArray,
			}
...
func (svc *Service) setTargetTypePerPipeline(pipelineID string, useTargetTypeOfByteArray bool) {
	// load target type from service instance in case custom target type is used
	svc.runtime.TargetTypeMap[pipelineID] = common.NewAtomicTargetType(svc.targetType.Type())
	if useTargetTypeOfByteArray {
		svc.runtime.TargetTypeMap[pipelineID].Set(&[]byte{})
	}
}
...
func (svc *Service) SetDefaultFunctionsPipeline(transforms ...interfaces.AppFunction) error {
	...
	svc.setTargetTypePerPipeline(interfaces.DefaultPipelineId, svc.config.Writable.Pipeline.UseTargetTypeOfByteArray)
...
func (svc *Service) AddFunctionsPipelineForTopics(id string, topics []string, triggerType string, useTargetTypeOfByteArray bool, transforms ...interfaces.AppFunction) error {
	svc.setTargetTypePerPipeline(id, useTargetTypeOfByteArray)

	t := strings.ToUpper(triggerType)
	if t == internal.TriggerTypeHTTP {
		return errors.New("pipeline per topic only valid with EdgeX MessageBus, External MQTT, and Custom Trigger")
	}

	// If the TriggerType defined under PerTopicPipeline doesn't exist in the Trigger definition,
	// an error will be returned so that users can catch the configuration issue as early as they can.
	types := svc.config.Trigger.Types()
	if _, ok := types[t]; !ok {
		err := fmt.Errorf("the given trigger type %s could not be found in the [Trigger] configuration", triggerType)
		svc.lc.Error(err.Error())
		return err
	}
	...
	err := svc.runtime.AddFunctionsPipeline(id, topics, triggerType, useTargetTypeOfByteArray, transforms)
	if err != nil {
		return err
	}
...
func (svc *Service) Initialize() error {
	...
	svc.lc.Infof("Starting %s %s ", svc.serviceKey, internal.ApplicationVersion)
