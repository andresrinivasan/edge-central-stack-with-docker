...
func (b *simpleTriggerServiceBinding) Runtime() *runtime.GolangRuntime {
	return b.Service.runtime
}
...
func (mp *triggerMessageProcessor) Process(ctx interfaces.AppFunctionContext, envelope types.MessageEnvelope) error {
	...
	// Pass triggerType as empty string for custom trigger
	...
	targetData, err, isInvalidMessage := mp.serviceBinding.DecodeMessage(context, envelope, defaultPipelines[0].Id)
	if err != nil {
		if isInvalidMessage {
...
func (mp *triggerMessageProcessor) MessageReceived(ctx interfaces.AppFunctionContext, envelope types.MessageEnvelope, responseHandler interfaces.PipelineResponseHandler) error {
	...
	triggerType, ok := ctx.GetValue(internal.TriggerType)
	if !ok {
		return fmt.Errorf("trigger type not specified")
	}
	pipelines := mp.serviceBinding.GetMatchingPipelines(envelope.ReceivedTopic, triggerType)
	...
	for _, pipeline := range pipelines {
		...
		targetData, err, isInvalidMessage := mp.serviceBinding.DecodeMessage(appContext, envelope, pipeline.Id)
		if err != nil {
			if isInvalidMessage {
				mp.invalidMessagesReceived.Inc(1)
			}
