...
// These helper functions transform the parameters map of Pipeline configuration to actual parameters required by functions.
type Configurable struct {
	lc logger.LoggingClient
	// to reduce the duplicate MQTT broker config, EdgeXpert allows pipelines reuse the same MQTT config as defined in the trigger section,
	// which will be referenced as externalMqttConfig field
	externalMqttConfig sdkCommon.ExternalMqttConfig
}

// NewConfigurable returns a new instance of Configurable
// extMqttConfig represents the same MQTT config as defined in the trigger section
func NewConfigurable(lc logger.LoggingClient, extMqttConfig sdkCommon.ExternalMqttConfig) *Configurable {
	return &Configurable{
		lc:                 lc,
		externalMqttConfig: extMqttConfig,
	}
}
...
func (app *Configurable) Compress(parameters map[string]string) interfaces.AppFunction {
	...
	base64Encode, ok := parameters[Base64Encode]
	if !ok {
		base64Encode = "true"
	}
	base64EncodeValue, err := strconv.ParseBool(base64Encode)
	if err != nil {
		app.lc.Errorf(
			"Could not parse '%s' to boolean for '%s' parameter, %s",
			base64Encode, Base64Encode, err.Error())
		return nil
	}

	transform := transforms.Compression{Base64Encode: base64EncodeValue}
...
func (app *Configurable) Batch(parameters map[string]string) interfaces.AppFunction {
	...
	useRawDataType, ok := parameters[UseRawDataType]
	if !ok {
		useRawDataType = "true"
	}
	useRawDataTypeValue, err := strconv.ParseBool(useRawDataType)
	if err != nil {
		app.lc.Errorf(
			"Could not parse '%s' to boolean for '%s' parameter for Batch: %s",
			useRawDataType, UseRawDataType, err.Error())
		return nil
	}
	...
	case BatchByCount:
		...
		transform, err = transforms.NewBatchByCount(thresholdValue, useRawDataTypeValue)
		if err != nil {
			app.lc.Error(err.Error())
			return nil
		}
	...
	case BatchByTime:
		...
		transform, err = transforms.NewBatchByTime(timeInterval, useRawDataTypeValue)
		if err != nil {
			app.lc.Error(err.Error())
			return nil
		}
	...
	case BatchByTimeAndCount:
		...
		transform, err = transforms.NewBatchByTimeAndCount(timeInterval, thresholdValue, useRawDataTypeValue)
		if err != nil {
			app.lc.Error(err.Error())
			return nil
		}
...
func (app *Configurable) processTagsParameter(parameters map[string]string) (map[string]interface{}, bool) {
	...
	if !ok {
		app.lc.Errorf("Could not find '%s' parameter", Tags)
		return nil, true
	}
