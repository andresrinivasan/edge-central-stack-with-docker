...
func Test_triggerMessageProcessor_MessageReceived(t *testing.T) {
	...
	ctx := appfunction.NewContext("", nil, "")
	...
	tests := []struct {
	...
	}{
		{
			name:    "no pipelines",
			setup:   returns{},
			args:    args{envelope: types.MessageEnvelope{CorrelationID: uuid.NewString(), ContentType: uuid.NewString(), ReceivedTopic: uuid.NewString()}, ctx: ctx},
			wantErr: 0,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			tsb.On("DecodeMessage", mock.Anything, mock.Anything, mock.Anything).Return(nil, nil, false)
			tsb.On("GetMatchingPipelines", tt.args.envelope.ReceivedTopic, mock.Anything).Return(tt.setup.pipelineMatcher)
			tsb.On("LoggingClient").Return(lc)
			tsb.On("BuildContext", mock.Anything).Return(tt.args.ctx)
			...
			if !tt.nilRh {
				rh = func(ctx interfaces.AppFunctionContext, pipeline *interfaces.FunctionPipeline) error {
					assert.NotSame(t, tt.args.ctx, ctx)
					assert.Equal(t, tt.args.ctx.CorrelationID(), ctx.CorrelationID()) //hmm
					return nil
				}
			}

			tt.args.ctx.AddValue(internal.TriggerType, mock.Anything)
