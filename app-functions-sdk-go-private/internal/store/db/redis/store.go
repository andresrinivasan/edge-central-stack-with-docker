...
func NewClient(config interfaces.DatabaseInfo, credentials bootstrapConfig.Credentials, tlsConfig *tls.Config) (interfaces.StoreClient, error) {
	var retErr error
	connectTimeout, retErr := time.ParseDuration(config.Timeout)
	if retErr != nil {
		return nil, fmt.Errorf("config.Timeout failed to parse: %v", retErr)
	}
	once.Do(func() {
		...
		if os.Getenv("EDGEX_SECURITY_SECRET_STORE") == common.ValueTrue {
			opts = append(opts, redis.DialPassword(credentials.Password))
