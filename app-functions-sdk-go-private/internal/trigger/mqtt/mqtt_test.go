...
func TestTrigger_messageHandler(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ctx := &interfaceMocks.AppFunctionContext{}
			ctx.On("AddValue", mock.Anything, mock.Anything).Return(mock.Anything)
			ctx.On("SetSharedMQTTClient", mock.Anything)
			ctx.On("SetSharedMQTTClientMutex", mock.Anything)
			ctx.On("SetMqttConnectionWaitingCounter", mock.Anything)
			...
			testRuntime := runtime.NewGolangRuntime("", sdkCommon.NewAtomicTargetType(nil), dic)
			serviceBinding.On("Runtime").Return(testRuntime)
