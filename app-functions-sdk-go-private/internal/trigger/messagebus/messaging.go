...
func (trigger *Trigger) Initialize(appWg *sync.WaitGroup, appCtx context.Context, background <-chan interfaces.BackgroundMessage) (bootstrap.Deferred, error) {
	...
	trigger.serviceBinding.Dic().Update(di.ServiceConstructorMap{
...
func (trigger *Trigger) messageHandler(logger logger.LoggingClient, _ types.TopicChannel, message types.MessageEnvelope) {
	...
	appContext.AddValue(internal.TriggerType, internal.TriggerTypeMessageBus)
	appContext.SetSharedMQTTClient(&trigger.serviceBinding.Runtime().SharedMQTTClient)
	appContext.SetSharedMQTTClientMutex(&trigger.serviceBinding.Runtime().SharedMQTTClientMutex)
	appContext.SetMqttConnectionWaitingCounter(trigger.serviceBinding.Runtime().MqttConnectionWaitingCounter)
...
func (trigger *Trigger) setOptionalAuthData(messageBusConfig *types.MessageBusConfig, lc logger.LoggingClient) error {
	...
	if err != nil {
		return fmt.Errorf("unable to get Secret Data for secure message bus: %v", err)
	}

	if err := bootstrapMessaging.ValidateSecretData(authMode, secretName, secretData); err != nil {
		return fmt.Errorf("secret Data for secure message bus invalid: %v", err)
	}
