...
func (trigger *Trigger) requestHandler(writer http.ResponseWriter, r *http.Request) {
	...
	appContext.AddValue(internal.TriggerType, internal.TriggerTypeHTTP)

	defaultPipeline := trigger.serviceBinding.GetDefaultPipeline()
	targetData, decodeErr, _ := trigger.serviceBinding.DecodeMessage(appContext.(*appfunction.Context), envelope, defaultPipeline.Id)
	if decodeErr != nil {
